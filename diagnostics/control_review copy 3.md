# 📊 Enterprise Control Execution Dashboard
...
| **❌ FAIL** | 9 (90.0%) |
| **🔴 ERROR** | 0 (0.0%) |
| **Total Records Tested** | 95,872 |
| **Total Exceptions Detected** | 5,204 |
...
|------------|------------------|-------|------------|--------------|----------|
| CTRL-AML-404 | 3 | 2 | 3 | 3 | ✗ |
| CTRL-CASS-006 | 1 | 0 | 1 | 1 | ✗ |
| CTRL-ECOA-908101 | 0 | 0 | 0 | 0 | ✗ |
| CTRL-IAM-007 | 3 | 2 | 3 | 3 | ✗ |
| CTRL-ITGC-001 | 2 | 1 | 2 | 2 | ✗ |
| CTRL-MNPI-707 | 2 | 1 | 2 | 2 | ✗ |
| CTRL-OPS-T2-003 | 1 | 0 | 1 | 1 | ✗ |
| CTRL-SOX-AP-004 | 2 | 1 | 1 | 2 | ✗ |
| CTRL-TRD-WASH-005 | 1 | 0 | 1 | 1 | ✗ |
| CTRL-VRM-999 | 4 | 2 | 2 | 3 | ✗ |

## SQL Complexity Metrics

| Control ID | CTEs | Joins | WHERE | IS NOT TRUE | EXCLUDE | Lines |
|------------|------|-------|-------|-------------|---------|-------|
| CTRL-AML-404 | 3 | 2 | 1 | 3 | 2 | 22 |
| CTRL-CASS-006 | 1 | 0 | 1 | 1 | 0 | 9 |
| CTRL-ECOA-908101 | 0 | 0 | 0 | 0 | 0 | 0 |
| CTRL-IAM-007 | 3 | 2 | 1 | 3 | 2 | 22 |
| CTRL-ITGC-001 | 2 | 1 | 1 | 2 | 1 | 14 |
| CTRL-MNPI-707 | 2 | 1 | 1 | 2 | 1 | 14 |
| CTRL-OPS-T2-003 | 1 | 0 | 1 | 1 | 0 | 6 |
| CTRL-SOX-AP-004 | 2 | 1 | 1 | 1 | 1 | 14 |
| CTRL-TRD-WASH-005 | 1 | 0 | 1 | 1 | 0 | 6 |
| CTRL-VRM-999 | 3 | 2 | 1 | 2 | 2 | 22 |

## Assertion Type Distribution

- **value_match**: 9 assertions
- **temporal_date_math**: 3 assertions
- **column_comparison**: 3 assertions
- **aggregation**: 1 assertions

## Quality Checks

⚠️ **1 potential issues detected:**
...
| **Exceptions Found** | 978 |
| **Exception Rate** | 100.00% |
| **Timestamp** | 2026-02-15T12:57:08.941046 |
...
- **Assertions:** 3 (value_match, value_match, temporal_date_math)

- **Data Sources:** onboarding_log_sheet1, ofac_watch_list_sheet1, edd_tracker_sheet1
...
**Column Names:** customer_id, edd_completion_date, analyst_id
...
**Column Names:** tax_id, sanction_entity_name, listed_date
...
**Column Names:** customer_id, tax_id, onboarding_date, risk_rating, account_status
...
```json
{
  ...
  "ontology_bindings": [
    ...
    {
      "business_term": "sanctioned tax identifier",
  ...
  "assertions": [
    {
      "assertion_id": "assertion_ofac_match",
      "description": "Ensure no onboarded customer exists on the OFAC Sanctions list.",
      "materiality_threshold_percent": 0.0,
      "assertion_type": "value_match",
      "field": "join_ofac_tax_id",
      "operator": "eq",
      "expected_value": null,
      "ignore_case_and_space": true
    },
    {
      "assertion_id": "assertion_edd_exists",
      "description": "For high-risk customers, ensure EDD exists in tracker.",
      "materiality_threshold_percent": 0.0,
      "assertion_type": "value_match",
      "field": "join_edd_customer_id",
      "operator": "neq",
      "expected_value": null,
      "ignore_case_and_space": true
    },
    {
      "assertion_id": "assertion_edd_timing",
      "description": "For high-risk customers, ensure EDD completed within 14 days of onboarding.",
      "materiality_threshold_percent": 0.0,
...
join_ofac AS (
    SELECT base.*,
           ...
           right_tbl.tax_id AS join_ofac_tax_id
    FROM base
...
join_edd AS (
    SELECT join_ofac.*,
           ...
           right_tbl.customer_id AS join_edd_customer_id
    FROM join_ofac
    LEFT JOIN read_parquet('data\parquet\edd_tracker_sheet1.parquet') AS right_tbl
    ON join_ofac.tax_id = right_tbl.customer_id
)

SELECT *
FROM join_edd
WHERE (risk_rating = 'HIGH') 
  AND ((join_ofac_tax_id IS NULL) IS NOT TRUE OR (join_edd_customer_id IS NOT NULL) IS NOT TRUE OR (CAST(edd_completion_date AS DATE) <= CAST(onboarding_date AS DATE) + INTERVAL 14 DAY) IS NOT TRUE)
```
...
| **Exceptions Found** | 1 |
| **Exception Rate** | 0.03% |
| **Timestamp** | 2026-02-15T12:57:21.886297 |
...
- **Data Sources:** client_money_ledger_sheet1
...
**Rows:** 3,000 | **Columns:** 4


**Column Names:** calculation_date, account_id, account_type, current_balance
...
```json
{
  "governance": {
    ...
    "risk_objective": "Ensure adequate segregation and funding of Client Money accounts to meet regulatory minimum buffer requirements"
  },
  "ontology_bindings": [
    {
  ...
  "assertions": [
    {
      "assertion_id": "agg_minimum_buffer_check",
      "description": "Verify that the aggregated SUM of current_balance grouped by calculation_date is strictly greater than or equal to $50,000,000 (mandatory minimum segregated buffer)",
      "materiality_threshold_percent": 0.0,
...
**Column Names:** record_id, application_id, application_date, letter_type, system_requested_count, ic_group_printed_count, reconciliation_status, bref_raised_flag, resolution_date
...
| **Exceptions Found** | 2 |
| **Exception Rate** | 0.04% |
| **Timestamp** | 2026-02-15T12:58:02.935125 |
...
- **Assertions:** 3 (value_match, temporal_date_math, value_match)

- **Data Sources:** system_accounts_sheet1, service_tickets_sheet1, hr_terminations_sheet1
...
**Rows:** 5,000 | **Columns:** 3


**Column Names:** employee_id, termination_date, termination_type
...
**Rows:** 5,000 | **Columns:** 4


**Column Names:** ticket_id, employee_id, ticket_status, closed_date
...
**Column Names:** account_id, employee_id, last_login
...
```json
{
  ...
  "ontology_bindings": [
    ...
    {
      "business_term": "Service Ticket Status",
    ...
    {
      "business_term": "System Account Employee ID",
  ...
  "assertions": [
    {
      "assertion_id": "assert_ticket_status_closed",
      "description": "Verify that the service ticket status is 'CLOSED'",
      "materiality_threshold_percent": 0.0,
      "assertion_type": "value_match",
      "field": "ticket_status",
      "operator": "eq",
    ...
    {
      "assertion_id": "assert_ticket_closed_within_1_day",
    ...
    {
      "assertion_id": "assert_no_system_account",
      "description": "Verify that the employee does NOT exist in the Tier 1 application (system_accounts)",
      "materiality_threshold_percent": 0.0,
      "assertion_type": "value_match",
      "field": "join_system_accounts_employee_id",
      "operator": "eq",
      "expected_value": null,
      "ignore_case_and_space": true
    }
  ...
  "evidence": {
    "retention_years": 7,
    "reviewer_workflow": "Requires_Human_Signoff",
    "exception_routing_queue": "IAM Access Control Exceptions"
  }
}
```
...
join_service_tickets AS (
    SELECT base.*,
           ...
           right_tbl.employee_id AS join_service_tickets_employee_id
    FROM base
...
join_system_accounts AS (
    SELECT join_service_tickets.*,
           right_tbl.* EXCLUDE (employee_id),
           right_tbl.employee_id AS join_system_accounts_employee_id
...
WHERE (termination_type IN ('VOLUNTARY', 'INVOLUNTARY')) 
  AND ((TRIM(UPPER(CAST(ticket_status AS VARCHAR))) = TRIM(UPPER('CLOSED'))) IS NOT TRUE OR (CAST(closed_date AS DATE) <= CAST(termination_date AS DATE) + INTERVAL 1 DAY) IS NOT TRUE OR (join_system_accounts_employee_id IS NULL) IS NOT TRUE)
```
...
| **Exceptions Found** | 1,380 |
| **Exception Rate** | 10.27% |
| **Timestamp** | 2026-02-15T12:58:19.867757 |
...
- **Assertions:** 2 (value_match, value_match)

- **Data Sources:** hr_master_roster_sheet1, active_directory_sheet1
...
**Rows:** 15,001 | **Columns:** 3


**Column Names:** system_account_id, employee_id, account_status
...
**Rows:** 15,000 | **Columns:** 2


**Column Names:** employee_id, employment_status
...
```json
{
  "governance": {
    ...
    "risk_objective": "Prevent unauthorized system access by terminated employees or unknown users to mitigate cybersecurity and data exfiltration risks."
  },
  "ontology_bindings": [
    {
  ...
  "assertions": [
    {
      ...
      "description": "Verify that the matching HR employment_status is NOT 'TERMINATED'.",
      "materiality_threshold_percent": 0.0,
      "assertion_type": "value_match",
    ...
    },
    {
      ...
      "description": "Ensure that the HR employment_status IS NOT NULL (to detect Ghost Accounts).",
      "materiality_threshold_percent": 0.0,
      "assertion_type": "value_match",
      "field": "employment_status",
      "operator": "neq",
  ...
  "evidence": {
    "retention_years": 7,
    "reviewer_workflow": "Requires_Human_Signoff",
    "exception_routing_queue": "ITGC Access Control Exceptions"
  }
}
```
...
join_hr_roster AS (
    SELECT base.*,
           right_tbl.* EXCLUDE (employee_id),
           right_tbl.employee_id AS join_hr_roster_employee_id
    FROM base
...
| **Exceptions Found** | 277 |
| **Exception Rate** | 1.85% |
| **Timestamp** | 2026-02-15T12:58:41.442762 |
...
- **Assertions:** 2 (value_match, column_comparison)

- **Data Sources:** personal_trade_blotter_sheet1, wall_cross_register_sheet1
...
**Rows:** 15,000 | **Columns:** 5


**Column Names:** trade_id, employee_id, ticker_symbol, trade_date, action
...
**Column Names:** employee_id, ticker_symbol, cross_date, restriction_status, clearance_date
...
```json
{
    ...
    "steps": [
      ...
      },
      {
        ...
        "action": {
          ...
          "field": "join_wall_cross_register_employee_id",
          "is_null": false
        }
      }
    ],
  ...
  "assertions": [
    {
      "assertion_id": "assertion_1",
      "description": "If an employee executed a trade for a ticker they were wall-crossed on, the restriction_status must be 'CLEARED'.",
      "materiality_threshold_percent": 0.0,
      "assertion_type": "value_match",
    ...
    {
      "assertion_id": "assertion_2",
      "description": "If the status is 'CLEARED', verify that the trade_date occurred strictly AFTER the clearance_date.",
      "materiality_threshold_percent": 0.0,
...
join_wall_cross_register AS (
    SELECT base.*,
           ...
           right_tbl.employee_id AS join_wall_cross_register_employee_id, right_tbl.ticker_symbol AS join_wall_cross_register_ticker_symbol
    FROM base
...
WHERE (join_wall_cross_register_employee_id IS NOT NULL) 
...
| **Exceptions Found** | 1 |
| **Exception Rate** | 0.01% |
| **Timestamp** | 2026-02-15T12:58:55.094001 |
...
- **Assertions:** 1 (temporal_date_math)

- **Data Sources:** equity_settlements_sheet1
...
**Rows:** 20,000 | **Columns:** 4


**Column Names:** trade_id, trade_date, settlement_date, trade_status
...
```json
{
  "governance": {
    ...
    "risk_objective": "Mitigate counterparty credit risk and avoid regulatory fines by ensuring equity trades settle within T+2 window"
  },
  "ontology_bindings": [
    {
  ...
  "assertions": [
    {
      ...
      "description": "Verify settlement_date occurs on or before trade_date + 2 days",
      "materiality_threshold_percent": 0.0,
      "assertion_type": "temporal_date_math",
...
| **Exceptions Found** | 2,562 |
| **Exception Rate** | 42.87% |
| **Timestamp** | 2026-02-15T12:59:09.926261 |
...
- **Assertions:** 1 (value_match)

- **Data Sources:** employee_titles_sheet1, ap_invoices_sheet1
...
**Column Names:** invoice_id, invoice_amount, approver_id
...
**Column Names:** approver_id, approver_title
...
```json
{
  ...
  "evidence": {
    "retention_years": 7,
    "reviewer_workflow": "Requires_Human_Signoff",
    "exception_routing_queue": "SOX_AP_Exceptions"
  }
}
```
...
join_employee_titles AS (
    SELECT base.*,
           ...
           right_tbl.approver_id AS join_employee_titles_approver_id
    FROM base
...
| **Exceptions Found** | 1 |
| **Exception Rate** | 0.00% |
| **Timestamp** | 2026-02-15T12:59:23.452223 |
...
- **Assertions:** 1 (column_comparison)

- **Data Sources:** daily_execution_blotter_sheet1
...
**Rows:** 25,000 | **Columns:** 4


**Column Names:** execution_id, status, buyer_account_id, seller_account_id
  ...
  "assertions": [
    {
      ...
      "description": "Verify buyer_account_id does not equal seller_account_id",
      "materiality_threshold_percent": 0.0,
      "assertion_type": "column_comparison",
...
| **Exceptions Found** | 2 |
| **Exception Rate** | 0.03% |
| **Timestamp** | 2026-02-15T12:59:45.380354 |
...
- **Data Sources:** vendor_master_sheet1, security_assessments_sheet1, active_contracts_sheet1
...
**Column Names:** contract_id, vendor_id, contract_status, renewal_date
...
**Rows:** 4,049 | **Columns:** 4


**Column Names:** assessment_id, vendor_id, assessment_status, expiration_date
...
**Rows:** 8,000 | **Columns:** 3


**Column Names:** vendor_id, vendor_name, criticality
...
```json
{
  "governance": {
    ...
    "risk_objective": "Prevent engagement with critical vendors lacking valid security assessments"
  },
  "ontology_bindings": [
    {
  ...
  "assertions": [
    ...
    {
      "assertion_id": "assertion_2",
      "description": "Verify that expiration_date is strictly AFTER renewal_date",
      "materiality_threshold_percent": 0.0,
      "assertion_type": "column_comparison",
...
join_vendor_master AS (
    SELECT base.*,
           ...
           right_tbl.vendor_id AS join_vendor_master_vendor_id
    FROM base
...
join_security_assessments AS (
    SELECT join_vendor_master.*,
           right_tbl.* EXCLUDE (vendor_id),
           right_tbl.vendor_id AS join_security_assessments_vendor_id
